...
public class ListarOrdemDeServicoDoClienteUseCase {
	...
	public ListarOrdemDeServicoDoClienteUseCase(OrdemDeServicoGateway ordemDeServicoGateway,
	...
	public void listar(int page, int size) {
		...
		var cliente = clienteGateway.buscarPorCpf(user.getCpf().getValor()).orElseThrow(ClienteNaoEncontradoException::new);

		outputPort.apresentar(ordemDeServicoGateway.listarPorClienteId(cliente.getId(), page, size).map(this::enriquecer));
	}
